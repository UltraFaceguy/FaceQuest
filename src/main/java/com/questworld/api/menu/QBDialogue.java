...
public class QBDialogue {
  ...
  public static void openDeletionConfirmation(Player p, final DataObject q) {
    ...
    menu.put(2, new ItemBuilder(Material.LIME_WOOL)
            .wrapText("&aYes I am sure", "", "&rThis will delete", tag).get(),
        event -> {
          Player p2 = (Player) event.getWhoClicked();
          ...
          if (q instanceof ICategory) {
            ...
            if (CancellableEvent.send(new CategoryDeleteEvent(category))) {
              ...
              QuestBook.openCategoryList(p2);
              PlayerTools
                  .sendTranslation(p2, true, Translation.CATEGORY_DELETED, category.getName());
            }
  ...
  public static void openResetConfirmation(Player p, final IQuest q) {
    ...
    menu.put(2, new ItemBuilder(Material.LIME_WOOL)
            .wrapText("&aYes I am sure", "", "&rThis will reset this Quest's Database").get(),
        event -> {
  ...
  public static void openQuestMissionEntityEditor(Player p, final IMission mission) {
    ...
    for (int i = 0; i < entities.length; ++i) {
      ...
      pager.addButton(i, EntityTools.getEntityDisplay(entity)
              .wrapText("&7Entity Type: &e" + EntityTools.nameOf(entity), "", "&e> Click to select")
              .get(),
          event -> {
  ...
  public static void openQuestMissionWaypointEditor(Player p, final IMission mission) {
    QuestWorld.getSounds().EDITOR_CLICK.playTo(p);

    IMissionState changes = mission.getState();
    final Menu menu = new LinkedMenu(6, "&3Waypoint selector", mission, true);

    PagedMapping pager = new PagedMapping(45);

    pager.addButton(0, new ItemBuilder(Material.RED_WOOL).display("&cCLEAR WAYPOINT")
            .wrapText("&cCLEAR WAYPOINT", "&e> Click to select").get(),
        event -> {
          changes.setWaypointerId("");
          changes.apply();
          QuestBook.openQuestMissionEditor((Player) event.getWhoClicked(), mission);
        }, true);
    int index = 1;
    for (Waypoint w : WaypointerPlugin.getInstance().getWaypointManager()
        .getWaypointsByDistance(p.getLocation())) {
      pager.addButton(index, new ItemBuilder(Material.NETHER_STAR).display("&b" + w.getId())
              .wrapText("&fWaypoint: " + w.getId(), "&e> Click to set this waypoint").get(),
          event -> {
            changes.setWaypointerId(w.getId());
            changes.apply();
            QuestBook.openQuestMissionEditor((Player) event.getWhoClicked(), mission);
          }, true);
      index++;
  ...
  public static void openCommandEditor(Player p, IQuest quest) {
    ...
    PlayerTools.tellraw(p,
        new JsonBlob("+ ", DARK_GREEN, add).add("Add more Commands... (Click)", GRAY, add)
            .toString());
    ...
    PlayerTools.tellraw(p,
        new JsonBlob("< ", BLUE, back).add("Return to quest editor", GRAY, back).toString());

    p.sendMessage(Text.colorize("&7&m----------------------------"));
  }

  private static boolean test(ArrayDeque<IQuest> backlog, HashSet<IQuest> collision,
      DataObject changing) {
  ...
  private static boolean cycleDetection(DataObject changing, DataObject peek) {
    ...
    IQuest parent =
        peek instanceof IQuest ? ((IQuest) peek).getParent() : ((ICategory) peek).getParent();
  ...
  private static void openRequirementQuests(Player p, final DataObject q, ICategory category) {
    ...
    for (IQuest quest : category.getQuests()) {
      if (!cycleDetection(q, quest)) {
        pager
            .addButton(quest.getID(), new ItemBuilder(quest.getItem()).wrapText(quest.getName(), "",
                "&e> Click to set requirement for " + (isQuest ? "quest" : "category") + ": &f&o"
                    + name).get(),
                event -> {
                  Player p2 = (Player) event.getWhoClicked();
        ...
        pager.addButton(quest.getID(),
            new ItemBuilder(Material.BARRIER).wrapText(quest.getName(), "", "&c> Requirement cycle")
                .get(),
            null, false);
      }
    }
