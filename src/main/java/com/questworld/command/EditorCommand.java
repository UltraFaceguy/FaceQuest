...
public class EditorCommand implements CommandExecutor {
  ...
  public void checkCmd(CommandSender sender, String label, String[] args) {
    if (args.length > 1) {
      String targetName = args[1];
      Player aaaa = PlayerTools.getPlayer(targetName);
      PlayerStatus status = (PlayerStatus) QuestWorld.getPlayerStatus(aaaa.getUniqueId());
      for (ICategory category : api.getFacade().getCategories()) {
        for (IQuest quest : category.getQuests()) {
          if (quest.isEnabled()) {
            status.checkComplete(quest, aaaa);
          }
        }
      }
      sender.sendMessage("Checked quests for " + aaaa.getName());
    }
  }
  ...
  public void resetCmd(CommandSender sender, String label, String[] args) {
    if (args.length > 1) {
      ...
      if (uuid != null) {
        ...
        sender.sendMessage(Text.colorize("&cReset quest progress for " + uuid));
      } else {
